...
def is_strictly_remote(title, desc):
    remote_aliases = [r"віддален", r"дистанційн", r"remote", r"home-office", r"вдома", r"дом[ау]", r"online", r"онлайн"]
...
def format_description(text):
    ...
    sections = {
        ...
        "🎁 Умови:": [r"умов", r"пропонуєм", r"графік"]
    }
    formatted = text
...
def run_scanner(pages, filters):
    ...
    for p in range(1, pages + 1):
        ...
        try:
            r = requests.get(f"https://www.work.ua/jobs-remote/?page={p}", timeout=15, headers={"User-Agent": "Mozilla/5.0"})
            soup = BeautifulSoup(r.text, "html.parser")
            ...
            st.sidebar.error(f"Помилка на ст. {p}: {e}")
            continue
...
with st.sidebar:
    ...
    st.divider()
    pages = st.number_input("Сторінок", 1, 15, 3)
    if st.button("🚀 Почати пошук", use_container_width=True):
        # Виклик функції всередині блоку з даними
...
if "df" in st.session_state and not st.session_state["df"].empty:
    ...
    for _, row in view.sort_values("match", ascending=False).iterrows():
        ...
        with st.expander(f"{row['match']}% — {row['title']}"):
            ...
            <div class="desc-text">{format_description(row['desc'])}</div>
            <div style="margin-top:15px;"><a href="{row['link']}" target="_blank" style="color:#2563eb; font-weight:700; text-decoration:none;">🔗 Відкрити на Work.ua</a></div>
            """, unsafe_allow_html=True)
else:
    st.info("👈 Налаштуйте фільтри та натисніть 'Почати пошук'.")
