...
def get_job_description(url):
    try:
        # Використовуємо Google Proxy замість прямого запиту
        r = requests.get(f"{GAS_PROXY_URL}?url={url}", timeout=20)
...
def run_scanner(pages, filters):
    ...
    for p in range(1, pages + 1):
        status.caption(f"Обробка сторінки {p} через Google Proxy...")
        try:
            # Запит через Google Proxy
            main_url = f"https://www.work.ua/jobs-remote/?page={p}"
            r = requests.get(f"{GAS_PROXY_URL}?url={main_url}", timeout=25)
            
            if r.status_code != 200:
                st.sidebar.warning(f"⚠️ Помилка проксі на сторінці {p}.")
                continue
...
with st.sidebar:
    ...
    if st.button("🚀 ПОЧАТИ СКАНУВАННЯ", use_container_width=True):
        with st.spinner("Збираю дані через Google (не закривайте вкладку)..."):
...
if "df" in st.session_state:
    ...
    if df.empty:
        st.warning("🤷‍♂️ Жодної релевантної віддаленої вакансії не знайдено.")
    else:
